...
<?php $title = !$isTrash ? 'Thành viên - Danh sách thành viên' : 'Thành viên - Danh sách thùng rác'; ?>
...
<section class="content-header">
    <h1>
        Danh sách <?php echo !$isTrash ? 'thành viên' : 'thùng rác'; ?>
    </h1>
    <ol class="breadcrumb">
        <li><a href="{!! route('admin.index') !!}"><i class="fa fa-dashboard"></i> Dashboard</a></li>
        <li><a href="{!! route('admin.users.getList') !!}">Thành viên</a></li>
        <li class="active">Danh sách <?php echo !$isTrash ? 'thành viên' : 'thùng rác'; ?></li>
    </ol>
</section>
<section class="content">
    <div class="box box-primary">
        <div class="box-body" id="u-l-list">
            ...
            @if (Session::has('user-inactive-success-message'))
            <div class="alert alert-info alert-dismissable">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                <h4><i class="icon fa fa-check"></i> Thành công!</h4>
                {{ Session::get('user-inactive-success-message') }}
            </div>
            @elseif (Session::has('user-inactive-error-message'))
            <div class="alert alert-danger alert-dismissable">
                <button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>
                <h4><i class="icon fa fa-ban"></i> Lỗi!</h4>
                {{ Session::get('user-inactive-error-message') }}
            </div>
            @endif
            ...
            <div class="btn-wrapper col-xs-12">
                <div class="btn-group pull-right">
                    ...
                    <a href="{!! route('admin.users.getAdd') !!}">
                    ...
                    <?php $route = $isTrash ? 'admin.users.getList' : 'admin.users.getTrash'; ?>
                    ...
                    @if (!empty($list))
                        @foreach ($list as $index => $row)
                            <tr class="table-tr">
                                ...
                                <td class="table-td">
                                    <p>{{ $row['profile']['date_of_birth'] }}</p>
                                </td>
                                <td class="table-td">
                                    <p>
                                        <?php
                                        $gender = $row['profile']['gender'];
                                        echo $listGenders[$gender];
                                        ?>
                                    </p>
                                </td>
                                <td class="table-td">
                                ...
                                <td class="table-td">
                                    <p>
                                        @if ($row['is_deleted'] && isset($myPermission['admin.users.undo']))
                                        <a href="{!! route('admin.users.undo', $row['id']) !!}" class="btn btn-primary tooltips btn-xs" data-placement="top" data-original-title="Kích hoạt lại">
                                            <i class="fa fa-undo"></i>
                                        </a>
                                        @else
                                            @if (isset($myPermission['admin.users.getEdit']))
                                            <a href="{!! route('admin.users.getEdit', $row['id']) !!}" class="btn btn-primary tooltips btn-xs" data-placement="top" data-original-title="Sửa">
                                                <i class="fa fa-pencil"></i>
                                            </a>
                                            @endif

                                            @if (Auth::user()->id != $row['id'] && isset($myPermission['admin.users.inactive']))
                                            <a href="{!! route('admin.users.inactive', $row['id']) !!}" class="btn btn-danger tooltips btn-xs" data-placement="top" data-original-title="Khóa">
                                ...
                                <td class="table-td none">
                                    <p>{{ $row['profile']['employee_no'] }}</p>
                                </td>
                                <td class="table-td none">
                                    <p>{{ $row['profile']['social_security_no'] }}</p>
                                </td>
                                <td class="table-td none">
                                    <p>{{ $row['profile']['phone_no'] }}</p>
                                </td>
                                <td class="table-td none">
                                    <p>{{ $row['profile']['address'] }}</p>
                                </td>
                                <td class="table-td none">
                                    <p>{{ $row['profile']['story'] }}</p>
                                </td>
                            </tr>
                        @endforeach
                    @endif
...
$(function () {
    ...
    var oTable = $('#edt').DataTable({
        ...
        "aoColumnDefs": [
            ...
            {"bSearchable": false, "aTargets": [0, 8, 9, 10]},
        ],
        "aaSorting": [[0, 'asc']],
